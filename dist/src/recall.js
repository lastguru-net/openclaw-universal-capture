...
function safeSessionKeyFileName(sessionKey) {
    return `${Buffer.from(sessionKey, "utf8").toString("base64url")}.ndjson`;
}
export function resolveRecallFilePath(params) {
    return resolve(params.workspaceDir, params.recallFolder, safeSessionKeyFileName(params.sessionKey));
}
function parseRecallEntry(value) {
    if (!isRecord(value))
        return undefined;
...
export function parseRecallLines(text) {
    const entries = [];
...
async function readRecallFile(filePath) {
    try {
...
export async function writeRecallEntries(params) {
    ...
    const tmpPath = resolve(dirname(filePath), `.${basename(filePath)}.${process.pid}.${Date.now()}.tmp`);
    try {
...
function renderRecallEntry(entry) {
    return [
        `Timestamp: ${entry.timestamp}`,
...
export function createUniversalRecallTool(params) {
    return {
        ...
        description: "Recall recent request-response turns captured for the current OpenClaw session.",
        parameters: {},
        execute: async () => {
