...
function messageSenderUsername(message) {
    const record = asRecord(message);
...
function parseSessionKey(sessionKey) {
    ...
    const parsed = {};
    if (parts[1])
        parsed.agent = parts[1];
    if (parts[2])
        parsed.surface = parts[2];
    if (parts.length > 3) {
...
function filterAllows(filter, value) {
    if (filter.mode === "all")
        return true;
    if (!value)
...
function sessionKeyPassesFilters(sessionKey, config) {
    const session = parseSessionKey(sessionKey);
    return (filterAllows(config.agents, session.agent) &&
        filterAllows(config.surfaces, session.surface) &&
        filterAllows(config.channels, session.channelId));
}
function findCorrespondingUser(messages, assistantIndex) {
...
function findJsonFenceEnd(text, start) {
...
function consumeLeadingMetadataBlock(text, start, header) {
    if (!text.startsWith(header, start))
        return undefined;
    const fenceStart = start + header.length;
    if (!text.startsWith(JSON_FENCE_START, fenceStart))
        return undefined;
...
export function stripLeadingUntrustedMetadata(text) {
    const afterConversationInfo = consumeLeadingMetadataBlock(text, 0, CONVERSATION_INFO_HEADER);
    if (afterConversationInfo === undefined)
        return text;
    const afterSender = consumeLeadingMetadataBlock(text, afterConversationInfo, SENDER_HEADER);
    return text.slice(afterSender ?? afterConversationInfo);
}
export function selectCaptureEntries(params) {
    const entries = [];
    if (!sessionKeyPassesFilters(params.sessionKey, params))
        return entries;
...
function renderMetadataLines(metadata) {
    ...
    if (metadata.surface)
        lines.push(`Surface: ${metadata.surface}`);
    if (metadata.channel)
        lines.push(`Channel: ${metadata.channel}`);
    if (metadata.senderUsername)
        lines.push(`Sender: ${metadata.senderUsername}`);
    return lines;
}
